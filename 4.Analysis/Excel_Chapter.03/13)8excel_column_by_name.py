...
my_cloumns = ["Customer ID", "Purchase Date"]
...
with open_workbook(input_file) as workbook :
    ...
    for header_index in range(len(header_list)) :
        if header_list[header_index] in my_cloumns :
            header_list_index.append(header_index)
    for row_index in range(1, worksheet.nrows) :
        row_list = []
        for column_index in header_list_index :
