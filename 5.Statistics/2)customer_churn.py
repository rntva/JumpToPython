...
#                                     "account_length" : ["mean", "std"],
#                                     "custserv_calls" : ["mean", "std"]}))

# 새로운 변수 total_charge를 기준으로 그룹화한 뒤,
# 그룹별 통계량 구하기
...
print(factor_cut)
def get_stats(group) :
    return {"min" : group.min(), "max" : group.max(), "count" : group.count(), "meam" : group.mean(), "std" : group.std()}
grouped = churn.custserv_calls.groupby(factor_cut)
print(grouped.apply(get_stats).unstack())

# account_length 열의 사분위수를 기준으로 분할한 뒤,
# 그룹별 통계량 구하기
factor_qcut = pd.cut(churn.account_length, [0., 0.25, 0.5, 0.75, 1.])
grouped = churn.custserv_calls.groupby(factor_qcut)
print(grouped.apply(get_stats).unstack())

# intl_plan과 vmail_plan 열에 대한 이진형 지시변수를 만들고,
# churn열과 병합하여 새로운 데이터프레임을 생성하기
...
# total_charge를 사분위수로 분할하고, 이진형 지시변수를 만들고,
...
# print(at_means)
# print("P of churn when ind. vars are their mean : %.3f" %invers_logit(at_means))
#
...
# print("Probability of churn when account length changes by 1 : %.3f" %(invers_logit(cust_serv_mean) - invers_logit(cust_serv_mean_minus_one)))
