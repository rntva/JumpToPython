...
file_name + index + file_name_extension, encoding="cp949",mode='w',index=True)
    index = int(index)
    index += 1
    index = str(index)
    ...
    with open(top_dir_name + dir_path  + index_file_name, 'w') as file:
        file.write('2')
    nene_table.to_csv(top_dir_name + dir_path + sub_dir_name + '1' + dir_path + file_name + '1' +\
file_name_extension, encoding="cp949", mode='w', index=True)